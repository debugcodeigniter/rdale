<?php if(!isset($loginSection)){?>
	...
	{
		$perpageURL = base_url('manage/'.$this->controller).'/index/'.$parent_id;
	}
	else if($this->controller=="pimages")
	{
		$perpageURL = base_url('manage/'.$this->controller).'/index/'.$cat_id;
	}
	else if($this->controller=="inclusions")
	{
		$perpageURL = base_url('manage/'.$this->controller).'/index/'.$dest_id;
	}
	else{
		$perpageURL = base_url('manage/'.$this->controller).'/';
	}
	?>
	var controller = "<?php echo $perpageURL;?>";
  ...
  <!-- <script src="<?php echo ADMIN_ASSETS;?>js/jvectormap/jquery-jvectormap-1.2.2.min.js"></script>
	...
	<?php } if(isset($galimagesScript) || isset($aimagesScript) || isset($photosScript) || isset($tourimagesScript) || isset($attractionimagesScript))
	{?>
	<!-- JavaScript Includes -->
	<script src="<?php echo ADMIN_ASSETS;?>js/jquery.knob.js"></script>
	<!-- jQuery File Upload Dependencies -->
    ...
    <script src="<?php echo ADMIN_ASSETS;?>js/jquery.barrating.min.js"></script>
    <script src="<?php echo ADMIN_ASSETS;?>js/jquery.sticky-kit.js"></script>
...
<div class="modal fade" id="addPackTagModal">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<label class="control-label">Tag Name :</label>
                  ...
                  <input type="text" name="pack_tag_ajax" id="pack_tag_ajax" value="" class="form-control aplha" placeholder="Pet Friendly" />
			</div>
            <span id="ptagErrorMsg" class="validate-has-error"></span>
			</div>
			
			<div class="modal-footer">
            <img src="<?php echo ADMIN_ASSETS;?>images/ajax_loader.gif" alt="Loading..." style="margin-right:10px;display:none" id="ajax_loader_ptag"/>
				<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
				<button type="button" class="btn btn-primary" id="pTagSubmit">Submit</button>
			</div>
		</div>
	</div>
</div>
