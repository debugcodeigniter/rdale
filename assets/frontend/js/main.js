...
$(function(){
	...
	$(".rd-factions").on('click',function(event){
		event.preventDefault();
		var boxID = $(this).attr('data-href');
		$(".rd-mforms").addClass('hidden');
		$(boxID).removeClass('hidden');
		
		var tPos = $(boxID).offset().top - $(".navbar").height();
		$("html, body").animate({scrollTop:tPos}, '500', 'swing');	
		console.log(boxID);
		if(boxID == "#reg-box")
		{
			ChangeUrl("Register", reg_url);	
			$(document).prop('title', 'Register');
		}else if(boxID == "#login-box")
		{
			ChangeUrl("Investor Login", login_url);	
			$(document).prop('title', 'Investor Login');
		}else if(boxID == "#forgot-box")
		{
			ChangeUrl("", forgot_url);	
			$(document).prop('title', 'Forgot Password');
		}
			
	});
	$("#page_contents img").addClass('img-responsive');
	
	$('.uname').keyup(function (event ) {
		
		 if(event.which == 37 || event.which == 38 || event.which == 39 || event.which == 40 ) { // Left / Up / Right / Down Arrow, Backspace, Delete keys
			 return;
		 }
	...
	{					
		...
		submitHandler: function() {
			...
			req = $.post(base_url+'home/doregister', 
			...
			function(data){
				...
				if(data.status=="true")
				{
					...
					$("#reg-error").html(data.message).removeClass('hidden');
				}
	...
	{					
			...
			submitHandler: function() {
				...
				req = $.post(base_url+'home/dologin', 
				$('form#logform').serialize(), 
				function(data){
					...
					if(data.status=="true")
					{
						...
						$("#log-error").html(data.message).removeClass('hidden');
					}
					//console.log(data);
					$("#log-loader").addClass('hidden');
	...
	{					
		...
		submitHandler: function() {
			$("#forgot-loader").removeClass('hidden');
			$("#forgot-error, #forgot-success").addClass('hidden');
			$("#forgot-btn").attr('disabled','disabled');
			if(req!=null)
			{
				req.abort();	
			}
			req = $.post(base_url+'home/doforgot', 
			$('form#forgotform').serialize() , 
			function(data){
				
				
				if(data.status=="true")
				{
					$("#forgot-success").html(data.message).removeClass('hidden');
				}
				else{
					$("#forgot-error").html(data.message).removeClass('hidden');
				}
				$("#forgot-loader").addClass('hidden');
	...
	{					
		...
		submitHandler: function() {
			$("#pwd-loader").removeClass('hidden');
			$("#pwd-error").addClass('hidden');
			$("#pwd-success").addClass('hidden');
			$("#pwd-btn").attr('disabled','disabled');
			if(req!=null)
			{
				req.abort();	
			}
			req = $.post(base_url+'home/dopwd', 
			...
			function(data){
				...
				if(data.status=="true")
				{
					$("#pwd-success").html(data.message).removeClass('hidden');
					$("#pwdform input[type=password]").val('');
				}
				else{
					$("#pwd-error").html(data.message).removeClass('hidden');
				}
				$("#pwd-loader").addClass('hidden');
	...
	{					
		...
		submitHandler: function() {
			...
			function(data){
				...
				if(data.status=="true")
				{
					$("#edit-success").html(data.message).removeClass('hidden');
				}
				else{
					$("#edit-error").html(data.message).removeClass('hidden');
				}
...
$(window).load(function(){
	...
	setTimeout(function(){
		$("#loader_logo").addClass('hide-loader-logo');
		$('body').addClass('loaded');
	}, 300);	
});

function setRdHeight()
{
	var winHeight = ($(window).height() > 600) ? $(window).height() : 600;
	$(".rd-cover").css('min-height',winHeight+'px');		
}
