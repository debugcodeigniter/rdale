jQuery( document ).ready(function( $ ) {
  ...
  $(".select2").on('change',function(){
		...
		{
			$("#s2id_" + this.id).css('border-color','#cc2424');
			$("#"+this.id+"-error").css('display','block');
			
		}else{
			$("#s2id_" + this.id).css('border-color','');
	...
	*/
	$("body").on('keyup','.dec',function(event){
		if(event.which == 37 || event.which == 38 || event.which == 39 || event.which == 40 ) { // Left / Up / Right / Down Arrow, Backspace, Delete keys
			 return;
		 }
     this.value = this.value.replace(/[^0-9\.]/g,'');
	...
	$("body").on('keyup','.numb',function(event){
		if(event.which == 37 || event.which == 38 || event.which == 39 || event.which == 40 ) { // Left / Up / Right / Down Arrow, Backspace, Delete keys
			 return;
		 }
	...
	$(".doslug").on('keyup',function(event){
		if(event.which == 37 || event.which == 38 || event.which == 39 || event.which == 40 ) { // Left / Up / Right / Down Arrow, Backspace, Delete keys
			 return;
		 }
		var input = this.value;
		var slugID = "#"+$(this).attr('data-slug-object');
		input = input.replace(/^\s\s*/, '') // Trim start
        .replace(/\s\s*$/, '') // Trim end
        .toLowerCase() // Camel case is bad
        .replace(/[^a-z0-9_\-~\+\s]+/g, '') // Exchange invalid chars
	...
	$("#page_name").on('keyup',function(event){
		if(event.which == 37 || event.which == 38 || event.which == 39 || event.which == 40 ) { // Left / Up / Right / Down Arrow, Backspace, Delete keys
			 return;
		 }
		var input = this.value;
	...
	$("#blog_name").on('keyup',function(event){
		if(event.which == 37 || event.which == 38 || event.which == 39 || event.which == 40 ) { // Left / Up / Right / Down Arrow, Backspace, Delete keys
			 return;
		 }
		var input = this.value;
	...
	$("#bCatSubmit").on('click',function(){
		...
		req = $.ajax({
			url: admin_url+'bcategories/addRecordAJAX',
			type: "POST",
	...
	$("#bTagSubmit").on('click',function(){
		...
		req_tag = $.ajax({
			url: admin_url+'btags/addRecordAJAX',
			type: "POST",
	...
	//For opening the blog category modal START
	$("#addNewPTag").on('click',function(){
		$("#addPackTagModal").modal('show');
		$("#ptagErrorMsg").html('');
	});
	//For opening the blog category modal END
	...
	$("input[type=text], texarea").attr('spellcheck','true');
